...
/* 
    ...
    changeBody.php is used to get the path for the next body image in the database.
...
//current body ID
$currentBodyID = filter_input(INPUT_GET, "currentBodyID", FILTER_VALIDATE_INT);

//Boolean Flag to check parameters
$paramsOK =false;

//Check if parameters OK
if($currentBodyID !== false && $currentBodyID !== null){
    $paramsOK = true;
}

//ID of the first body image
$firstId;
//ID of the last body image
$lastId;
//NextNum is the ID after the current one
$nextNum = $currentBodyID + 1;
...
if($paramsOK){
    ...
    $data = array((object)["body"=>$body,"currentBodyID"=>$nextNum]);


//Return body image path and ID
